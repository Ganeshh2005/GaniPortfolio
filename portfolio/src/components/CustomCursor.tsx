...
export default function CustomCursor() {
    ...
    return (
        <>
            <div
                ref={cursorRef}
                className="fixed top-0 left-0 w-8 h-8 rounded-full pointer-events-none z-[9999] -ml-4 -mt-4"
                style={{ border: '1px solid var(--sakura-pink)', backgroundColor: 'transparent' }}
            ></div>
            <div
                ref={cursorDotRef}
                className="fixed top-0 left-0 w-2 h-2 rounded-full pointer-events-none z-[9999] -ml-1 -mt-1"
                style={{ backgroundColor: 'var(--gold-warm)', boxShadow: '0 0 10px rgba(232, 149, 42, 0.4)' }}
            ></div>
        </>
    );
}
